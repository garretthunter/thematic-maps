...
//		$this->loader = $loader;
	...
	public function add_plugin_links( $links, $plugin_file ) {
		...
		$plugin_links = array(
			'<a href="' . admin_url( 'admin.php?page=thematic_maps_plugin' ) . '">' . __( 'Settings', 'thematic_maps_plugin' ) . '</a>',
		);
	...
	/*  ============================================================================
		Following methods handle the global plugin settings page and options
	    ============================================================================ */
	/**
	 ...
	 * @return array
	 */
	public function set_default_plugin_options()
	{
	...
	public function tm_plugin_settings_init () {
		...
		add_settings_field(
			...
			$this->plugin_name.'_plugin',	            // The page on which this option will be displayed
			$this->plugin_name.'_plugin'			        // The name of the section to which this field belongs
		);

		register_setting(
			$this->plugin_name.'_plugin',					// Settings group name
			$this->plugin_name.'_plugin',						// Option to save
			array( $this, 'validate_options_plugin_settings')   // Sanitize callback
		);

	}

	/**
	 * This function displays the global plugin settings page
	 *
	 * @since 1.1.0
	 *
	 * @param string $active_tab
	 */
	public function render_tm_plugin_page( $active_tab = 'plugin' ) {

		$active_tab = 'plugin';
		require_once plugin_dir_path( __FILE__ ) . 'partials/tm-plugin-page.php';

	}

	/**
	 * This function displays a description under the page title
	 *
	 * @since 1.1.0
	 */
	public function plugin_settings_description_callback () {
...
//		}
		/**
		 * Save the original options until the input is validated
		 */
//		$current_options = get_option( $this->plugin_name . '_plugin' );
//		$new_options     = array();
//
...
//		return apply_filters( 'validate_options_plugin_settings', $new_options, $input );
	...
	/*  ============================================================================
		Following methods handle the new map page and settings
		============================================================================ */
	/**
...
//			return apply_filters( 'validate_options_new_map', $new_options, $input );
	...
	private function has_valid_nonce() {
		...
		$action = 'validate_plugin_settings';
